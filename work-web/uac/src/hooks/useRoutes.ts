...
import { HOME_NAME, LOGIN_URL, homeRouter, notFoundRouter } from "@/router/routesConfig";
...
import { listRoutes, type Menu } from "@/api/system/menu";
import { LAYOUT_NAME, uacAppSecret } from "@work/constants";
...
export const useRoutes = () => {
  ...
  const initDynamicRouters = async (roles?: string[], api?: () => Promise<Menu.MenuInfo[]>) => {
    ...
    } else if (api) routeList = await api();
    else {
  ...
  const loadDynamicRouters = (routers: RouterConfigRaw[], roles: string[], r = router) => {
    ...
    // 最后添加 notFoundRouter
    router.addRoute(notFoundRouter as RouteRecordRaw);
  };
  /**
